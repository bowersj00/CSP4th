...
def find_solutions():
    p1points = set(path1_points)
    p2points = set(path2_points)
    for p1point in p1points:
        if p1point in p2points:
            print("found: "+p1point)
            solutions.append(p1point)
    for solution in solutions:
        print(path1_vals[solution]+path2_vals[solution])


def move(direction, path):
    global path1x , path1y , path2x , path2y , path1_points , path2_points , path1_steps , path2_steps
    ...
    if path==1:
        path1_steps+=1
        value=str(path1x)+","+str(path1y)
        path1_points.add(value)
        path1_vals[value]=path1_steps
    else:
        path2_steps+=1
        value=str(path2x)+","+str(path2y)
        path2_points.add(value)
        path2_vals[value]=path2_steps
...
for first_direction in path1:
    for i in range(0,int(first_direction[1:])):
        move(first_direction[0],1)
    path2x=0
    path2y=0
for second_direction in path2:
    for i in range(0,int(second_direction[1:])):
        move(second_direction[0],2)
print("finding solutions")   
print(len(path1_points),len(path2_points))
find_solutions()        
print(solutions)